...
/// Final screen: the top scorer (or a shared top score), then the standings.
class MostLikelyToGameOverView extends StatelessWidget {
  const MostLikelyToGameOverView({required this.session, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    if (winner != null) {
      ...
      subtitle = 'Most likely to... everything, apparently.';
    } else if (leaders.isEmpty) {
      headline = 'No points scored!';
      subtitle = 'Nobody pointed at anyone. Suspiciously polite.';
    } else {
      headline = "It's a tie!";
            ...
            children: [
              Expanded(
                child: OutlinedButton(
                  onPressed: () => context.go(AppRoutes.mostLikelyTo),
  ...
  static String _points(int score) => score == 1 ? '1 pt' : '$score pts';
}
